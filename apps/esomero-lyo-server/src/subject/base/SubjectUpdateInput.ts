/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CourseUpdateManyWithoutSubjectsInput } from "./CourseUpdateManyWithoutSubjectsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { TeacherUpdateManyWithoutSubjectsInput } from "./TeacherUpdateManyWithoutSubjectsInput";

@InputType()
class SubjectUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CourseUpdateManyWithoutSubjectsInput,
  })
  @ValidateNested()
  @Type(() => CourseUpdateManyWithoutSubjectsInput)
  @IsOptional()
  @Field(() => CourseUpdateManyWithoutSubjectsInput, {
    nullable: true,
  })
  courses?: CourseUpdateManyWithoutSubjectsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => TeacherUpdateManyWithoutSubjectsInput,
  })
  @ValidateNested()
  @Type(() => TeacherUpdateManyWithoutSubjectsInput)
  @IsOptional()
  @Field(() => TeacherUpdateManyWithoutSubjectsInput, {
    nullable: true,
  })
  teachers?: TeacherUpdateManyWithoutSubjectsInput;
}

export { SubjectUpdateInput as SubjectUpdateInput };
